        ...
        {listing?.length > 0
          ? listing.map((item, index) => (
              <Link href={"/view-listing/" + item.id} key={index}>
                <div className="p-3 border border-transparent hover:border-2 hover:border-[#7f57f1] transition-all duration-300 cursor-pointer rounded-lg h-full flex flex-col">
                  {/* <Image
                    src={item.listingImages?.[0]?.url || "/placeholder-building.jpg"}
                    width={800}
                    height={150}
                  ...
                  <Image
                    src={
                      item?.listingImages && item.listingImages.length > 0
                        ? (typeof item.listingImages[0] === "string"
                            ? item.listingImages[0]
                            : item.listingImages[0]?.url) || "/placeholder-building.jpg"
